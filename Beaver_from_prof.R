...
#temp (dependent var) = not normally distributes
#activity (independent var) = normally distributes

#format of test is wilcox.test
#wilcox.test(dependent ~ independent)
wilcox.test(temp ~ activity)

#p-value < 0.05 so the null hypothesis is rejected
# and it is concluded that the beaver body temperature is affected by activity (p = 2.2e-16)
